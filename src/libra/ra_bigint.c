...
count_bits(const struct ra_bigint *a)
{
        ...
        return i * 64 + (64 - ra_clz(a->parts[i]));
}
...
convert_real(double r)
{
        ...
        if ((LLONG_MIN < r) && (LLONG_MAX > r)) {
                if (!(z = convert_int(llround(r)))) {
                        RA_TRACE("^");
                        return NULL;
                }
                return z;
        ...
        // get exponent and mantissa, adding the implicit 53rd bit to mantissa
